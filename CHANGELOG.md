...
### Changed
- Remap flow now **hides products whose options are already mapped** so they aren't listed for re-mapping. `app/api/remap/match` builds each row's best-matched SRS option block (via the same `buildOptionBlock` the apply step writes) and drops the product when it has ≥1 SRS option value and **every** one is already present (case-insensitive) on the Zuper product's existing `option.option_values` — Zuper may carry extra options. Products whose matched SRS product has no options stay listed (nothing to map). Because the product-*list* scan doesn't return the option block on every data-center, rows that could be already-mapped but came back optionless are backfilled via a bounded-concurrency per-product GET (the same call the apply step trusts); if the list already supplies options, no GET is made. The match stream now emits an `alreadyMapped` count, threaded through the store (`remapAlreadyMapped`) and shown in the Review header ("N products already mapped — hidden").

### Added
- **`scripts/clone-template-add-accessories.mjs`** — clones a CPQ proposal template and inserts an "Accessories" section (after Material) with the formula-backed accessory parts. Mirrors the `create-proposals` build path (POST template → POST options → PUT config → per-option replay sections/items), reading each source line item's `product.product_uid`/`formula.formula_uid` to re-post it. Clone is created as a **draft with no trigger** (two CPQ templates on the same category/status trigger collide); aborts if the target name already exists.
- **One-off account scripts** (`scripts/add-accessories.mjs`, `scripts/add-accessory-formulas.mjs`, `scripts/link-accessory-formulas.mjs`) for manually adding roofing-accessory PARTS to a single account, creating checklist-driven CPQ formulas for them, and linking each product to its formula. All three resolve the base URL from the company login (same as `/api/connect`), are idempotent (skip-by-name/key), and support `--dry-run`. Notable: a **CHECKLIST**-type `expression_map` entry (`{key, type:'CHECKLIST', field_uid, field_name}`) lets a CPQ formula read a checklist answer directly (vs. a measurement token) — `field_uid` is the question's `checklist_uid` from `GET settings/checklist`. Linking a product to a formula requires PUTting the **embedded formula object** (`{formula_uid, formula_name, formula_key, formula:{expression, expression_map, rounding_mechanism}}`) onto `product.formula`, not a bare UID; the link script GET-then-PUTs and normalizes Zuper's nested GET shape (category/location objects → UID strings) so existing fields are preserved.
- Uploaded products now carry **SIZE** options, not just colors. `lib/product-builder.ts` gained `resolveOptions()`, which picks the option axis from a product's variants: both color & size vary → **composite** values (`Black — 36" x 144'`) from the *real* variant combinations (not a cartesian); only color varies → color; only size varies → size; constant color + varying sizes loads the sizes (doesn't collapse to the one color). Previously size-only products (e.g. impact-rated VELUX skylights, ducting, flashing that vary by size) uploaded with an empty option block. Size/composite are non-mandatory; SHINGLES color stays mandatory. Mirrors the standalone account backfill (`product importer/backfill-account-options.js`). `buildOptionBlock`/`buildProductPayload` take an `includeSize` flag — **QXO passes `false`** because it overloads `size_name` with its UOM (color-only, unchanged); SRS/ABC pass `true`. Composite >50 (Zuper's hard option cap) falls back to the largest single axis that fits. The vendor-catalog SKU map in `app/api/upload/route.ts` is now keyed by the *chosen* option label (`variantCodeByOption` via `resolveOptions().labelOf`) instead of color only, so SKU↔option_uid links stay correct for size/composite products too.

### Fixed
- Remap "Apply" failed every product with Zuper's generic "Error in Updating Product Details". `app/api/remap/apply` did a GET-then-PUT but spread the **raw GET response** back into the PUT, and Zuper's GET nests `product_category` / `location_availability[].location` / `uom` / `formula` as objects (and tags `meta_data` rows with a server `_id`) — a shape its PUT rejects wholesale. New `lib/zuper-product-put.ts` (`buildRemapPutProduct`) flattens those to UID strings and strips `_id` before swapping in the new option block, preserving every other field. Mirrors the proven normalization in `scripts/link-accessory-formulas.mjs`.
- Idempotency scan used the wrong product-list endpoint on some data-centers: it hit `{baseUrl}products` (plural), which 404s on regions that serve the list at `product` (singular, e.g. `us-west-1c`). The 404 was swallowed (`Existing-product scan failed` warning) so the scan found nothing existing and a wizard **re-run POSTed duplicates** instead of PUT-updating. `app/api/upload/route.ts` now resolves the working list segment from page 1 — tries `product`, falls back to `products` on 404 — and reuses it for the paged fan-out.
- Product options (colors/sizes) were missing on most uploaded products: the variant fetch in `app/api/upload/route.ts` chunked product_ids by 500 but did **not** paginate, so PostgREST's 1000-row cap silently truncated each chunk. A 500-product chunk averages ~4,600 variants → only the first 1,000 returned, so ~77% of products in each chunk reached `buildProductPayload` with **zero variants** → empty `option` block, even when they had colors. New `fetchVariantRows()` helper pages each id-chunk with `.range()` ordered by the variant PK (unique → deterministic windows) until drained; applied to all three source branches (`srs_variants`, `abc_variants`, `qxo_variants`). The products fetch was already safe (one row per id). No payload/schema change.
...
### Added
- **Remap options for existing products** — a second flow alongside the import wizard. A new landing page (`app/page.tsx` → `mode` switch in the store) offers "Import SRS catalog" (the existing wizard, unchanged) or "Remap options for existing products". The remap flow (`components/remap/RemapShell.tsx`, 4 steps: Connect · Match · Review · Done) logs into Zuper, live-fetches the account's existing products, fuzzy-matches each against the SRS catalog, and on confirm writes **only** the matched color/size option block back onto each product (name/price/category/description preserved via GET-then-PUT). Matching is a TypeScript port of the audited `product importer/match-account-to-srs.js` scoring core (`lib/srs-match.ts`); the review table buckets matches exact/strong/weak/none with strong-and-up pre-selected, alternative-pick, and per-row skip. Two SSE routes: `app/api/remap/match` (scan → score → option rollup) and `app/api/remap/apply` (GET → merge option → PUT). Products already carrying a stamped SRS `product_id` (prior import) short-circuit to an exact match.
...
- **G/B/B packages now fall back to curated universal products for Underlayment and Ice & Water** when the brand carries none under its own name. Live data showed QXO Gaf/Certainteed/Tamko/Malarkey have zero I&W items (it's a GCP/Henry/OC commodity), so every QXO proposal shipped without ice & water; the curated accessory-catalog I&W products existed but were filtered out of the universal maps. New `MAP_COMPONENTS` superset feeds the maps; `buildPackage` falls back per-slot (`app/api/proposal-preview/route.ts`).
- **QXO flagship patterns populated** in `lib/flagship-lines.ts` (was an empty "v2.1 follow-up" stub) — Gaf Timberline HD/HDZ/UHDZ/Natural Shadow, CertainTeed Landmark, OC Duration/Oakridge/TruDefinition, spot-checked against live `qxo_products.product_line` (®/™ marks sit on word boundaries, so the ABC regexes match unchanged). QXO proposals now bubble the flagship shingle instead of natural row order.
- **Per-brand skip reasons in proposal preview.** When a brand fails the ≥2-shingle-tier gate (or has no products in the selected lines), proposal-preview now returns `__skipped: [{brand, reason}]` and Step9/Step10 render amber notices — replacing the unexplained generic "No eligible roofing brands found" dead end.
- **Upload warnings surfaced.** The price-fallback computation and per-product color-option GETs previously failed into bare `catch {}` (products silently priced at $0; vendor catalog silently losing SKU↔color mappings). Both now emit SSE `{type:'warning'}` events rendered as a banner in Step 6.

### Changed
- Extracted three reusable helpers from `app/api/upload/route.ts` so the upload and remap flows share one implementation (no behavior change to upload): `fetchAllZuperProducts()` (`lib/zuper-products.ts`, the paginated existing-product scan), `fetchVariantRows()`/`fetchSrsVariants()` (`lib/srs-variants.ts`, the paginated variant fetch), and `buildOptionBlock()` (`lib/product-builder.ts`, the colors→option-block construction).
- Hardened `app/api/preview/route.ts`: brand names are double-quoted inside the PostgREST `.or(in.(...))` filter (commas/parens in a future brand name can no longer corrupt the filter) and all accessory fetches now throw on Supabase errors instead of continuing with empty data.
- Removed the dead `__unsupported` handler in Step10Proposals (the proposal-preview route stopped returning that marker in v0.6.0).
- Raised proposal-preview query caps with measured-size comments: universal accessories 50→200 (catalogs are 13-21 ids), brand roofing query 500→1000 (largest brand today is 199 rows).

### Fixed
- ABC and QXO products no longer all upload to Zuper with UOM "EA". ABC: `abc_products.product_uom` is now populated upstream (costing UOM, e.g. SQ for shingles) and flows through the existing `toZuperUom()` path. QXO: the upload route derived nothing (`product_uom: null` hardcoded) even though `qxo_variants.uom` is 95% populated — it now tallies each product's variant UOMs (first segment of pipe-delimited values like "PLT|BDL") and stamps the most common. `UOM_MAP` gains `BDL` (QXO's bundle spelling); unmapped exotic codes still fall back to EA.
